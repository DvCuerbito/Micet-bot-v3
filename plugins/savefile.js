...
export default {
    ...
    async execute(sock, msg, { args, config, startTime, isOwner, pushName, userNumber, isGroup, replyWithContext, isBotSelf }) {
        ...
        if (!isOwner) {
            return await replyWithContext(`𔖮๋ׅꉹ᮫ִׁ۫🍔𐋕᮫ִׁ᷂𐴲᮫ִׁ╾᳞҇┄⵿֟፝━᮫๋࣭݁ El comando \`${config.prefix}savefile\` no existe.\n> Usa ${config.prefix}help para ver mis comandos`);
        }
        
        // Verificar si se proporcionó nombre de archivo
        if (!args || args.length === 0) {
            return await replyWithContext(`𔖮๋ׅꉹ᮫ִׁ۫🍟𐋕᮫ִׁ᷂𐴲᮫ִׁ╾᳞҇┄⵿֟፝━᮫๋࣭݁ *Debes proporcionar el nombre de un archivo y responder a un texto*\n> *Ejemplo* » ${config.prefix}savefile plugins/ping.js\n> ${config.prefix}savefile index.js`);
        }
        
        // Verificar si se respondió a un mensaje
        if (!quoted) {
            return await replyWithContext(`𔖮๋ׅꉹ᮫ִׁ۫🍟𐋕᮫ִׁ᷂𐴲᮫ִׁ╾᳞҇┄⵿֟፝━᮫๋࣭݁ *Debes responder a un texto*`);
        }
        ...
        if (!texto.trim()) {
            return await replyWithContext(`𔖮๋ׅꉹ᮫ִׁ۫🍟𐋕᮫ִׁ᷂𐴲᮫ִׁ╾᳞҇┄⵿֟፝━᮫๋࣭݁ *El texto está vacío*`);
        }
        ...
        if (fileName.includes('..')) {
            return await replyWithContext(`𔖮๋ׅꉹ᮫ִׁ۫🍟𐋕᮫ִׁ᷂𐴲᮫ִׁ╾᳞҇┄⵿֟፝━᮫๋࣭݁ *Nombre no válido*\n> No se permiten rutas con '..'`);
        }
